import os, time, datetime, math, logging, argparse, torch
import torch.optim as optim
import torch.backends.cudnn as cudnn
import torch.utils.data as data
from data.dataset.dog_nose import DogNoseDetection, detection_collate
...
from utils.data_loader import data_prefetcher

LOG_FORMAT = "%(asctime)s %(levelname)s : %(message)s"
logging.basicConfig(level=logging.INFO, format=LOG_FORMAT)

save_dirname = "20230823"
pretrained_dirname = "20230822"

parser = argparse.ArgumentParser(description='Training')
parser.add_argument('--training_dataset', default='../../datasets/nosedataforyolo/label.txt', help='Training dataset directory')
...
parser.add_argument('--resume_net', default=f'weights/{pretrained_dirname}/Blaze_Final.pth', help='resume net for retraining')
#parser.add_argument('--resume_net', default=None, help='resume net for retraining')
parser.add_argument('--resume_epoch', default=0, type=int, help='resume iter for retraining')
parser.add_argument('--weight_decay', default=5e-4, type=float, help='Weight decay for SGD')
parser.add_argument('--gamma', default=0.1, type=float, help='Gamma update for SGD')
parser.add_argument('--save_folder', default=f'weights/{save_dirname}/', help='Location to save checkpoint models')
...
with torch.no_grad():
    ...
    priors = priors.cuda()
# import numpy as np
# priors = torch.tensor(np.load("anchors.npy"), dtype=torch.float32, device=torch.device('cuda'))
...
def train():
    ...
    dataset = DogNoseDetection(training_dataset, preproc(img_dim, rgb_mean))
