...
def encode_landm(matched, priors, variances):
    ...
    matched = torch.reshape(matched, (matched.size(0), 2, 2))
    priors_cx = priors[:, 0].unsqueeze(1).expand(matched.size(0), 2).unsqueeze(2)
    priors_cy = priors[:, 1].unsqueeze(1).expand(matched.size(0), 2).unsqueeze(2)
    priors_w = priors[:, 2].unsqueeze(1).expand(matched.size(0), 2).unsqueeze(2)
    priors_h = priors[:, 3].unsqueeze(1).expand(matched.size(0), 2).unsqueeze(2)
